#!/usr/bin/env python3
"""
harvest_school.py — the school wire: who owns schooling, who writes what is
taught, what it costs, and what it is for.

After the School subsection of Economic Inequality Within It: the factory model
and the workforce it was built to supply, curriculum homogenisation and the
local knowledge it displaces, the textbook oligopoly and its mergers, private
equity and the privatisation of public schooling, for-profit colleges and
diploma mills, student debt and credential inflation, the elite university
pipeline into political office, the capture of global education governance,
edtech contracts and vendor lock-in, student data as a corporate asset,
teachers replaced by technology, the testing industry, what may be taught, what
endowments invest in, who is kept out, and the learning built outside all of it.

A sports result or a prize day is not this subject. How school is owned,
governed, priced, standardised or contracted out is. Every subject term carries
the schooling or accountability words it must appear beside.
...
SOURCES_PATH = os.path.join(HERE, "sources_school.json")
OUT_PATH = os.path.join(HERE, "wire_school.json")
...
# --------------------------------------------------------------------------
USER_AGENT = ("Mozilla/5.0 (compatible; school-feed/1.0; "
...
# THE SUBJECTS
#
# Taken from the School subsection: who owns schooling, who writes what is
# taught, what it costs, and what it is for.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words, so a sports result or a prize day cannot reach the wire.
# --------------------------------------------------------------------------
TOPICS = [
    ("factory", "The factory model and its purpose", [
        ("factory model", ["school", "education", "classroom"]),
        ("prussian model", []), ("industrial model", ["school", "education"]),
        ("punctual", ["docile", "sober", "worker", "factory"]),
        ("compulsory schooling", ["origin", "history", "law", "act", "designed"]),
        ("banking model of education", []), ("hidden curriculum", []),
        ("bells", ["factory", "school day", "regiment"]),
        ("rote learning", ["criticis*", "reform", "replace", "system"]),
        ("obedience", ["school", "classroom", "student", "trained"]),
        ("workforce", ["pipeline", "quota", "prepare", "trained", "school"]),
        ("school to work", []), ("work readiness", ["curriculum", "school"]),
        ("deschooling", []), ("unschooling", []),
    ]),
    ("homogenisation", "Curriculum homogenisation", [
        ("curriculum", ["standardis*", "standardiz*", "homogen*", "national", "reform", "overhaul", "imposed"]),
        ("global curriculum", []), ("international baccalaureate", []),
        ("cambridge international", []), ("caie", ["curriculum", "exam", "school"]),
        ("common core", []), ("national curriculum", ["reform", "change", "row", "impose", "review"]),
        ("standardised curriculum", []), ("standardized curriculum", []),
        ("one size fits all", ["education", "curriculum", "school"]),
        ("curriculum control", []), ("curriculum reform", []),
    ]),
    ("localknowledge", "Local and Indigenous knowledge", [
        ("indigenous knowledge", ["school", "curriculum", "education", "classroom"]),
        ("mother tongue", ["instruction", "education", "school", "policy", "ban"]),
        ("language of instruction", []), ("bilingual education", ["cut", "ban", "policy", "ruling"]),
        ("indigenous language", ["school", "curriculum", "taught", "revival", "loss"]),
        ("local knowledge", ["curriculum", "school", "erased", "excluded"]),
        ("decolonis*", ["curriculum", "education", "school"]),
        ("decoloniz*", ["curriculum", "education", "school"]),
        ("residential school", []), ("boarding school", ["indigenous", "assimilation", "abuse", "inquiry"]),
        ("cultural erasure", ["school", "education", "curriculum"]),
        ("land-based learning", []), ("traditional knowledge", ["school", "curriculum", "education"]),
    ]),
    ("publishers", "The textbook oligopoly", [
        ("pearson", ["education", "school", "curriculum", "textbook", "profit", "contract", "merger"]),
        ("mcgraw hill", []), ("houghton mifflin", []), ("cengage", []),
        ("textbook", ["price", "cost", "publisher", "market", "monopol", "contract", "adopt"]),
        ("instructional materials", ["contract", "market", "adopt", "publisher"]),
        ("curriculum publisher", []), ("education publisher", []),
        ("big three", ["textbook", "publisher", "education"]),
        ("scholastic", ["contract", "market", "profit", "censor"]),
    ]),
    ("consolidation", "Mergers and market consolidation", [
        ("merger", ["education", "edtech", "publisher", "school", "university", "curriculum"]),
        ("acquisition", ["edtech", "education company", "publisher", "learning platform"]),
        ("acquires", ["edtech", "education", "learning platform", "publisher"]),
        ("consolidation", ["education", "edtech", "publishing", "school market"]),
        ("antitrust", ["education", "edtech", "publisher", "textbook"]),
        ("nwea", []), ("market share", ["education", "edtech", "classroom", "school"]),
        ("monopoly", ["education", "textbook", "edtech", "curriculum"]),
    ]),
    ("privateequity", "Private equity in education", [
        ("private equity", ["education", "school", "university", "edtech", "childcare", "nursery", "college"]),
        ("apollo global", ["education", "university", "school"]),
        ("kkr", ["education", "school", "university", "edtech"]),
        ("blackstone", ["education", "school", "university", "student housing"]),
        ("veritas capital", []),
        ("leveraged buyout", ["education", "school", "college", "university"]),
        ("investor-owned", ["school", "college", "nursery"]),
        ("hedge fund", ["school", "education", "university", "endowment"]),
        ("asset manager", ["education", "school", "university"]),
    ]),
    ("privatisation", "Privatisation of public schooling", [
        ("public-private partnership", ["school", "education"]),
        ("privatisation", ["school", "education", "university"]),
        ("privatization", ["school", "education", "university"]),
        ("academisation", []), ("academy trust", []), ("multi-academy", []),
        ("charter school", ["expansion", "profit", "closure", "chain", "audit", "scandal"]),
        ("voucher", ["school", "education", "programme", "program"]),
        ("low-fee private school", []), ("bridge international academies", []),
        ("school chain", ["private", "for-profit", "expansion"]),
        ("outsourc", ["school", "education", "teaching", "district"]),
        ("public funds", ["private school", "voucher", "charter", "diverted"]),
    ]),
    ("forprofit", "For-profit colleges and diploma mills", [
        ("for-profit college", []), ("for-profit university", []),
        ("diploma mill", []), ("degree mill", []), ("fake degree", []),
        ("predatory", ["college", "school", "university", "recruit", "lending"]),
        ("accreditation", ["revoked", "lost", "denied", "fraud", "scandal", "withdrawn"]),
        ("closed abruptly", ["college", "university", "school"]),
        ("misled students", []), ("deceptive recruitment", []),
        ("borrower defense", []), ("loan discharge", ["college", "school", "fraud"]),
    ]),
    ("debt", "Student debt and the price of entry", [
        ("student debt", []), ("student loan", []), ("tuition", ["rise", "increase", "cost", "hike", "fee", "protest", "cap"]),
        ("tuition fees", []), ("fee increase", ["university", "college", "school"]),
        ("loan forgiveness", []), ("debt cancellation", ["student", "education"]),
        ("cost of attendance", []), ("graduate debt", []),
        ("school fees", ["barrier", "abolish", "burden", "out of school", "cannot afford"]),
        ("unaffordable", ["tuition", "school", "university", "education"]),
    ]),
    ("credential", "Credential inflation", [
        ("degree requirement", []), ("credential inflation", []),
        ("degree inflation", []), ("bachelor's degree", ["require", "jobs", "employers", "share of"]),
        ("degree apprenticeship", []), ("skills gap", ["narrative", "employers", "claim", "disputed"]),
        ("graduate underemployment", []), ("overqualified", ["graduates", "workers"]),
        ("employers drop", ["degree", "requirement"]),
    ]),
    ("elite", "The elite university pipeline", [
        ("elite university", ["pipeline", "leadership", "recruit", "over-represented", "civil service"]),
        ("oxbridge", []), ("ivy league", ["pipeline", "admissions", "elite", "influence", "recruit"]),
        ("sciences po", []), ("grandes écoles", []), ("grandes ecoles", []),
        ("legacy admissions", []), ("admissions scandal", []),
        ("social mobility", ["university", "school", "admissions", "elite"]),
        ("old boys", ["network", "school"]), ("private school", ["elite", "advantage", "over-represented", "cabinet", "judiciary"]),
        ("civil service", ["recruit", "graduates", "elite", "over-represented"]),
    ]),
    ("governance", "Capture of global education governance", [
        ("unesco", ["education", "curriculum", "funding", "influence", "partnership"]),
        ("global partnership for education", []), ("global education forum", []),
        ("multistakeholder", ["education", "governance"]),
        ("education policy", ["donor", "influence", "private actors", "shaped by"]),
        ("world bank", ["education", "school", "curriculum", "lending"]),
        ("oecd", ["education", "pisa", "school", "curriculum"]),
        ("pisa", ["ranking", "test", "influence", "policy", "criticis*"]),
        ("philanthropy", ["education", "school", "curriculum", "influence"]),
        ("gates foundation", ["education", "school", "curriculum"]),
        ("donor", ["education policy", "curriculum", "school reform"]),
    ]),
    ("regcapture", "Corporate partnership with the state", [
        ("lobbying", ["education", "school", "edtech", "curriculum", "university"]),
        ("industry partnership", ["school", "education", "curriculum", "district"]),
        ("advisory board", ["education", "curriculum", "corporate", "school"]),
        ("revolving door", ["education", "department", "regulator", "ministry"]),
        ("sponsored curriculum", []), ("corporate sponsorship", ["school", "classroom", "curriculum"]),
        ("memorandum of understanding", ["school", "ministry of education", "district"]),
        ("public contract", ["school", "education", "district", "ministry"]),
    ]),
    ("edtech", "Edtech contracts and vendor lock-in", [
        ("edtech", ["contract", "market", "spending", "procurement", "failure", "criticis*"]),
        ("vendor lock-in", []), ("multi-year contract", ["district", "school", "university"]),
        ("google classroom", []), ("chromebook", ["school", "district", "contract"]),
        ("microsoft 365", ["school", "education", "district"]),
        ("learning management system", []), ("procurement", ["school", "district", "education", "university"]),
        ("licence fee", ["school", "software", "district"]), ("license fee", ["school", "software", "district"]),
        ("subscription", ["school", "district", "textbook", "platform", "locked"]),
    ]),
    ("platform", "Knowledge behind a paywall", [
        ("proprietary platform", ["education", "school", "learning"]),
        ("open educational resources", []), ("open access", ["textbook", "education", "learning"]),
        ("paywall", ["research", "journal", "textbook", "learning", "course"]),
        ("digital textbook", ["access", "expire", "rental", "cost"]),
        ("access code", ["textbook", "homework", "course"]),
        ("inclusive access", ["textbook", "publisher"]),
        ("public domain", ["textbook", "curriculum", "educational"]),
    ]),
    ("data", "Student data as an asset", [
        ("student data", []), ("student privacy", []),
        ("edtech", ["privacy", "tracking", "data", "surveillance"]),
        ("data protection", ["school", "student", "pupil", "classroom", "university"]),
        ("biometric", ["school", "student", "classroom", "canteen", "attendance"]),
        ("facial recognition", ["school", "student", "campus", "classroom"]),
        ("proctoring", []), ("exam surveillance", []),
        ("tracking", ["student", "pupil", "classroom", "learning platform"]),
        ("data breach", ["school", "student", "university", "district", "education"]),
        ("children's data", []), ("profiling", ["student", "pupil", "learner"]),
    ]),
    ("brand", "Ecosystem conditioning", [
        ("brand", ["classroom", "school", "student", "loyalty", "curriculum"]),
        ("advertising", ["school", "classroom", "student", "campus"]),
        ("marketing to children", []), ("in-school marketing", []),
        ("free devices", ["school", "student", "district"]),
        ("certification programme", ["vendor", "student", "school"]),
        ("certification program", ["vendor", "student", "school"]),
        ("ecosystem", ["locked", "school", "student", "device"]),
    ]),
    ("automation", "Teachers replaced by technology", [
        ("replace teachers", []), ("teacher shortage", ["technology", "software", "platform", "automat"]),
        ("artificial intelligence", ["classroom", "school", "teaching", "curriculum", "marking", "grading"]),
        ("ai tutor", []), ("automated grading", []), ("automated marking", []),
        ("adaptive learning", []), ("personalised learning", ["platform", "algorithm", "software"]),
        ("personalized learning", ["platform", "algorithm", "software"]),
        ("algorithm", ["student", "grading", "placement", "school", "learning"]),
        ("screen time", ["school", "classroom", "policy", "device"]),
    ]),
    ("testing", "The testing industry", [
        ("standardised test", []), ("standardized test", []),
        ("high-stakes testing", []), ("test scores", ["policy", "funding", "ranking", "pressure", "gap"]),
        ("exam board", []), ("assessment", ["contract", "industry", "vendor", "market", "scandal"]),
        ("test prep", []), ("league table", ["school", "university"]),
        ("ranking", ["university", "school", "criticis*", "gamed", "methodology"]),
        ("teaching to the test", []), ("exam fraud", []), ("cheating scandal", ["exam", "test", "school"]),
    ]),
    ("teachers", "Teachers and their conditions", [
        ("teacher", ["strike", "pay", "shortage", "union", "workload", "quit", "walkout", "conditions"]),
        ("teachers union", []), ("educators strike", []),
        ("teaching assistant", ["pay", "cut", "conditions"]),
        ("academic staff", ["strike", "casual", "precarious", "pay", "cut"]),
        ("adjunct", []), ("casualisation", ["academic", "teaching", "university"]),
        ("casualization", ["academic", "teaching", "university"]),
        ("class size", ["rise", "cap", "protest", "policy"]),
    ]),
    ("censorship", "What may be taught", [
        ("book ban", []), ("banned books", ["school", "library", "classroom"]),
        ("curriculum ban", []), ("textbook censorship", []),
        ("textbook", ["rewritten", "revised", "removed", "censor", "history", "controvers"]),
        ("history curriculum", ["rewritten", "revised", "row", "dispute", "censor"]),
        ("academic freedom", []), ("teaching restricted", []),
        ("gag order", ["teacher", "school", "classroom"]),
        ("climate education", ["removed", "denied", "curriculum", "ban", "required"]),
        ("evolution", ["curriculum", "removed", "taught", "ban"]),
    ]),
    ("endowment", "What schools invest in", [
        ("endowment", ["fossil fuel", "divest", "invest", "arms", "holdings", "portfolio"]),
        ("divestment", ["university", "school", "endowment", "campus"]),
        ("fossil fuel", ["university", "endowment", "campus", "research funding", "sponsorship"]),
        ("research funding", ["oil", "arms", "tobacco", "industry", "conflict of interest"]),
        ("industry funding", ["university", "research", "department"]),
        ("arms company", ["university", "campus", "research", "recruit"]),
        ("recruitment", ["campus", "oil", "arms", "industry", "career fair"]),
        ("conflict of interest", ["university", "research", "academic"]),
    ]),
    ("access", "Who gets in and who is kept out", [
        ("out of school", ["children", "million", "crisis", "figures"]),
        ("school closure", ["cuts", "budget", "rural", "community", "protest"]),
        ("segregation", ["school", "education", "classroom"]),
        ("exclusion", ["school", "pupil", "student", "rate", "disproportionate"]),
        ("dropout", ["rate", "rise", "crisis"]), ("enrolment", ["fall", "drop", "gap", "barrier"]),
        ("enrollment", ["fall", "drop", "gap", "barrier"]),
    ...
    ("alternatives", "Learning outside the system", [
        ...
        ("democratic school", []), ("sudbury", ["school"]), ("montessori", ["policy", "expansion", "research"]),
        ("community school", ["run", "founded", "indigenous", "led"]),
        ("indigenous-led school", []), ("language nest", []),
        ("apprenticeship", ["alternative", "route", "instead of", "expansion"]),
        ("free school meals", ["campaign", "expansion", "cut"]),
        ("education reform", ["movement", "campaign", "grassroots", "teacher-led"]),
        ("student movement", []), ("school strike", ["climate", "student", "pupil"]),
    ]),
]
...
BLOCK = [
    # school life as event listing rather than school as a system
    "school sports", "varsity", "homecoming", "prom", "graduation ceremony",
    "school play", "science fair", "spelling bee", "bake sale", "field trip",
    "school bus crash", "snow day", "term dates", "half term", "back to school sale",
    "school lunch menu", "yearbook", "alumni reunion", "class of 20",
    "football", "basketball", "soccer match", "athletics meet", "cheerleading",
    "scholarship winner", "honour roll", "honor roll", "valedictorian",
    # ratings and marketing copy
    "best schools ranking sponsored", "top universities sponsored", "study abroad deals",
    "online course discount", "coupon", "black friday",
    # entertainment
    "school drama series", "teen movie", "campus comedy", "film review", "video game",
    # other wires
    "school shooting", "campus shooting", "police raid", "arrested on campus",
]
...
DECIDED = [
    ...
    "adopted", "mandated", "banned", "prohibited", "ordered", "settled",
    "acquired", "merged", "divested", "closed", "shut down", "stripped of accreditation",
]
INSTITUTIONAL = [
    "unesco", "unicef", "oecd", "world bank", "united nations", "european commission",
    "ministry of education", "department for education", "department of education",
    "education ministry", "school inspectorate", "ofsted", "auditor general",
    "national audit office", "government accountability office", "official gazette",
    "parliamentary committee", "select committee", "court filing", "regulator",
    "peer-reviewed", "published in", "study finds", "working paper", "dataset",
    "official data", "government figures", "national statistics", "pisa results",
    "global education monitoring report", "unesco institute for statistics",
]
MEASURED = [
    "per cent", "percent", "%", "billion", "million", "trillion",
    "share of gdp", "per capita", "per pupil", "per student", "enrolment rate",
    "enrollment rate", "pupil-teacher ratio", "figures show", "fell by", "rose by",
    "increase of", "decrease of", "estimated", "median", "average", "ranked", "index",
    "market share", "contract worth", "valued at",
]
PENDING = [
    "proposed", "draft law", "consultation", "under review", "tender", "bidding",
    "expected to", "due to decide", "talks", "negotiation", "in negotiations",
    "deadline", "next term", "next school year", "next year", "review scheduled",
    "pending approval", "pilot", "trial scheme", "green paper", "white paper",
]
...
LOCAL_TERMS = {
    "brand": [
        ("acordo de marca", None), ("acuerdo de marca", None),
        ("colossi tecnologici dispositivi", None), ("gigantes tecnológicas dispositivos", None),
        ("gigantes tecnológicos dispositivos", None), ("géants du numérique", None),
        ("it大手 学校 端末", None), ("marchi nelle aule", None),
        ("markenpräsenz im klassenzimmer", None), ("marque dans les", None),
        ("techkonzerne geräte kostenlos", None), ("教室 ブランド 進出", None),
        ("教室 品牌 植入", None), ("科技巨头 学校 免费设备", None),
        ("교실 브랜드 진출", None), ("빅테크 학교 기기", None),
    ],
    "censorship": [
        ("bücherverbote an schulen", None), ("censura curricular", None),
        ("censura dei programmi", None), ("censura del currículo", None),
        ("censure des programmes", None), ("divieto di libri", None),
        ("interdiction de livres", None), ("müfredat sansürü", None),
        ("okullarda kitap yasağı", None), ("pelarangan buku di", None),
        ("prohibición de libros", None), ("proibição de livros", None),
        ("sensor kurikulum", None), ("zensur im lehrplan", None),
        ("запрет книг в", None), ("цензура школьной программы", None),
        ("学校 図書 排除", None), ("学校 禁书", None),
        ("教育課程 検閲", None), ("课程 审查", None),
        ("교육과정 검열", None), ("학교 도서 금지", None),
    ],
    "consolidation": [
        ("acquisizione edtech antitrust", None), ("adquisición edtech competencia", None),
        ("aquisição edtech concorrência", None), ("fusion d'éditeurs scolaires", None),
        ("fusion schulbuchverlage", None), ("fusione editori scolastici", None),
        ("fusión de editoriales", None), ("fusão de editoras", None),
        ("rachat edtech concurrence", None), ("übernahme edtech kartellamt", None),
        ("покупка edtech компании", None), ("слияние образовательных издательств", None),
        ("エドテック 買収", None), ("教育出版 合并", None),
        ("教育出版 統合", None), ("教育科技 收购 反垄断", None),
        ("교육 출판사 합병", None), ("에듀테크 인수", None),
    ],
    "credential": [
        ("abschluss als einstellungsvoraussetzung", None), ("bildungsinflation", None),
        ("déclassement des diplômés", None), ("exigence de diplôme", None),
        ("exigência de diploma", None), ("inflación de credenciales", None),
        ("inflation des diplômes", None), ("inflazione dei titoli", None),
        ("inflação de credenciais", None), ("requisito de titulación", None),
        ("requisito di laurea", None), ("subempleo de titulados", None),
        ("subemprego de licenciados", None), ("unterbeschäftigung akademiker", None),
        ("инфляция дипломов", None), ("требование диплома отменено", None),
        ("大卒 不完全就業", None), ("学历贬值", None),
        ("学历门槛 取消", None), ("学歴インフレ", None),
        ("学歴要件 撤廃", None), ("毕业生 就业不足", None),
        ("학력 요건 폐지", None), ("학력 인플레이션", None),
    ],
    "debt": [
        ("aumento das propinas", None), ("aumento delle tasse", None),
        ("bildungskredit", None), ("crédito estudantil", None),
        ("debito studentesco", None), ("dette étudiante", None),
        ("deuda estudiantil", None), ("dívida estudantil", None),
        ("harç zammı", None), ("hausse des frais", None),
        ("kenaikan uang kuliah", None), ("préstamos estudiantiles", None),
        ("prêts étudiants", None), ("studiengebühren erhöhung", None),
        ("studienschulden", None), ("subida de las", None),
        ("utang mahasiswa", None), ("öğrenci borcu", None),
        ("öğrenim kredisi", None), ("рост платы за", None),
        ("студенческий долг", None), ("ارتفاع الرسوم الدراسية", None),
        ("ديون الطلاب", None), ("छात्र ऋण बोझ", None),
        ("फीस वृद्धि", None), ("奨学金 返済 負担", None),
        ("学生贷款 负担", None), ("学費 値上げ", None),
        ("学费 上涨", None), ("등록금 인상", None),
        ("학자금 대출 부담", None),
    ],
    "edtech": [
        ("collecte de données", None), ("datenerhebung edtech schulen", None),
        ("logiciels de surveillance", None), ("pengawasan siswa aplikasi", None),
        ("pengumpulan data edtech", None), ("raccolta dati edtech", None),
        ("recogida de datos", None), ("recolha de dados", None),
        ("software de vigilancia", None), ("software de vigilância", None),
        ("software di sorveglianza", None), ("überwachungssoftware schüler", None),
        ("сбор данных edtech", None), ("слежка за учениками", None),
        ("学生 监控 软件", None), ("教育アプリ 個人情報 学校", None),
        ("教育科技 数据收集 学校", None), ("生徒 監視 ソフト", None),
        ("에듀테크 학생 정보", None), ("학생 감시 소프트웨어", None),
    ],
    "elite": [
        ("admisiones por legado", None), ("admissões por legado", None),
        ("elitenreproduktion hochschule", None), ("eliteuniversitäten führungspositionen", None),
        ("grandes écoles et", None), ("reproduction des élites", None),
        ("riproduzione delle élite", None), ("sciences po dirigeants", None),
        ("universidades de elite", None), ("universidades de élite", None),
        ("università d'élite e", None), ("élite universitaria dirigentes", None),
        ("воспроизводство элит образование", None), ("элитные университеты и", None),
        ("エリート大学 出身 官僚", None), ("名校垄断 领导层", None),
        ("学歴 偏り 指導層", None), ("精英大学 出身 高官", None),
        ("엘리트 대학 출신", None), ("학벌 편중", None),
    ],
    "endowment": [
        ("campagna di disinvestimento", None), ("campagne de désinvestissement", None),
        ("campanha de desinvestimento", None), ("campaña de desinversión", None),
        ("divestment-kampagne hochschule", None), ("universidad inversiones en", None),
        ("universidade investimentos em", None), ("università investimenti in", None),
        ("universität investitionen fossile", None), ("université investissements énergies", None),
        ("кампания за отказ", None), ("университет инвестиции в", None),
        ("ダイベストメント 運動", None), ("大学 化石燃料 投資", None),
        ("大学 化石燃料 投资", None), ("撤资 运动 校园", None),
        ("대학 화석연료 투자", None), ("투자 철회 운동", None),
    ],
    "privatisation": [
        ("ausbau privater schulträger", None), ("ekspansi sekolah swasta", None),
        ("espansione delle scuole", None), ("expansion des écoles", None),
        ("expansión de escuelas", None), ("expansão de escolas", None),
        ("gestión privada de", None), ("gestão privada de", None),
        ("okulların özelleştirilmesi", None), ("privatisasi sekolah", None),
        ("privatisation de l'école", None), ("privatisierung von schulen", None),
        ("privatización escolar", None), ("privatização escolar", None),
        ("privatizzazione della scuola", None), ("özel okul teşviki", None),
        ("приватизация школ", None), ("расширение частных школ", None),
        ("توسع المدارس الخاصة", None), ("خصخصة المدارس", None),
        ("निजी स्कूलों का", None), ("स्कूल निजीकरण", None),
        ("学校 民営化", None), ("学校 民营化", None),
        ("民办学校 扩张", None), ("私立学校 拡大 公費", None),
        ("사립학교 확대", None), ("학교 민영화", None),
    ],
    "regcapture": [
        ("alianza curricular con", None), ("curricolo sponsorizzato", None),
        ("currículo patrocinado por", None), ("gesponserter lehrplan", None),
        ("parceria curricular com", None), ("partenariat entreprises programmes", None),
        ("partenariato aziendale programmi", None), ("programme sponsorisé par", None),
        ("unternehmenspartnerschaft lehrplan ministerium", None), ("корпоративное партнёрство учебная", None),
        ("企业 课程 合作", None), ("企业赞助 教材", None),
        ("企業 カリキュラム 連携", None), ("企業提供 教材", None),
        ("기업 교육과정 협력", None), ("기업 후원 교재", None),
    ],
    "teachers": [
        ("conflicto salarial del", None), ("conflit salarial enseignants", None),
        ("conflito salarial dos", None), ("greve por falta", None),
        ("grève des enseignants", None), ("huelga por escasez", None),
        ("kekurangan guru mogok", None), ("lehrermangel streik", None),
        ("sciopero degli insegnanti", None), ("sengketa gaji guru", None),
        ("spór płacowy nauczycieli", None), ("strajk nauczycieli", None),
        ("tarifstreit lehrkräfte", None), ("vertenza salariale docenti", None),
        ("öğretmen açığı grev", None), ("öğretmen maaş uyuşmazlığı", None),
        ("нехватка учителей забастовка", None), ("спор о зарплатах", None),
        ("نزاع رواتب المعلمين", None), ("نقص المعلمين إضراب", None),
        ("教員 待遇 改善", None), ("教員不足 ストライキ", None),
        ("教师 薪酬 争议", None), ("教师短缺 罢工", None),
        ("교사 부족 파업", None), ("교사 임금 분쟁", None),
    ],
    "testing": [
        ("critiche ai test", None), ("critiques des tests", None),
        ("críticas a las", None), ("críticas aos testes", None),
        ("kritik an standardisierten", None), ("kritik ujian standar", None),
        ("presión de los", None), ("pression des examens", None),
        ("pressione degli esami", None), ("pressão dos exames", None),
        ("prüfungsdruck schüler", None), ("standart sınavlara eleştiri", None),
        ("sınav baskısı öğrenciler", None), ("tekanan ujian siswa", None),
        ("критика стандартизированных тестов", None), ("экзаменационное давление на", None),
        ("परीक्षा का दबाव", None), ("मानकीकृत परीक्षा आलोचना", None),
        ("受験 プレッシャー 生徒", None), ("应试压力 学生", None),
        ("标准化考试 批评", None), ("標準テスト 批判", None),
        ("입시 압박 학생", None), ("표준화 시험 비판", None),
    ],
}
...
FILL_TERMS = {
    "factory": [
        ("fabrikmodell schule", None), ("modello fabbrica della", None),
        ("modelo fabril da", None), ("modelo fabril de", None),
        ("modèle industriel de", None), ("origem da escolaridade", None),
        ("origen de la", None), ("origine de l'école", None),
        ("origine dell'obbligo scolastico", None), ("preußisches modell bildung", None),
        ("ursprung der schulpflicht", None), ("义务教育 起源 工厂", None),
        ("工厂化 学校 模式", None), ("工場モデル 学校", None),
        ("義務教育 起源 工場労働者", None), ("공장식 학교 모델", None),
        ("의무교육 기원 공장", None),
    ],
    "platform": [
        ("code d'accès manuel", None), ("codice di accesso", None),
        ("código de acceso", None), ("código de acesso", None),
        ("offene bildungsressourcen einführung", None), ("recursos educativos abertos", None),
        ("recursos educativos abiertos", None), ("ressources éducatives libres", None),
        ("risorse educative aperte", None), ("zugangscode digitales lehrbuch", None),
        ("オープン教材 導入", None), ("デジタル教科書 利用期限", None),
        ("开放教育资源 推广", None), ("数字教材 使用期限", None),
        ("공개 교육자원 도입", None), ("디지털 교과서 이용", None),
    ],
}
...
def point_for(text, places, subs, regions, locale=None):
    ...
    if _GAZETTEER:
        glabel, gpoint, grank, _approx = galaxy_places.resolve_ranked(text)
        if grank == 3:
...
def run(dry_run=False, fixtures=None):
        ...
        if raw:
            ...
            for row in parse_feed(raw, src):
                ...
                row["pn"], row["ll"], row["pa"] = point_for(
                    text, places, subs, regions, src.get("gl"))
                row["p"] = total
                row["y"] = reasons
                row["st"] = src["standing"]
                if absorb(row):
    ...
    payload = {
        ...
        "languages": languages,
        "standings": [
            {"id": "official", "label": "Ministries, inspectorates & UN bodies"},
            {"id": "research", "label": "Research & education data"},
            {"id": "press", "label": "Press"},
            {"id": "rights", "label": "Unions & campaigning groups"},
        ],
    ...
    if dry_run:
        print("\n--dry-run: wire_school.json not written")
        return payload
